...
4. Taxonomic classification (Kraken2 on reads) - by nf-core/mag
5. **Additional: Contig classification** (Kraken2 on MEGAHIT and SPAdes contigs) - custom step
6. Report generation (MultiQC)

**Runtime:** ~5-10 hours (including contig classification)
...
├── Assembly/
│   ├── MEGAHIT/
│   │   └── MEGAHIT-group-viral_group1.contigs.fa.gz   # MEGAHIT assembly (28M)
│   └── SPAdes/
...
|---------|-------------|------------|
| Tool | nf-core/mag v2.5.4 + custom | Direct containers |
| Assembly | MEGAHIT + SPAdes | Flye |
| QC | fastp + FastQC | Skipped (NumPy issue) |
| Reads Classification | ✅ By nf-core/mag | ✅ Direct Kraken2 |
| Contigs Classification | ✅ Custom step (both assemblers) | ✅ Direct Kraken2 |
...
```bash
# Compare viral detection: Reads vs MEGAHIT vs SPAdes
echo "=== Reads Classification ==="
grep "Viruses" results_short/Taxonomy/kraken2/llnl_66ce4dde/llnl_66ce4dde.kraken2_report.txt

echo ""
echo "=== MEGAHIT Contigs Classification ==="
grep "Viruses" results_short/Taxonomy/contigs/llnl_66ce4dde_MEGAHIT_kraken2_report.txt

echo ""
echo "=== SPAdes Contigs Classification ==="
grep "Viruses" results_short/Taxonomy/contigs/llnl_66ce4dde_SPAdes_kraken2_report.txt
```
...
awk '$6~/Viruses/ {sum+=$2} END {print "Total viral reads:", sum}' \
    results_short/Taxonomy/kraken2/llnl_66ce4dde/llnl_66ce4dde.kraken2_report.txt

# Total viral contigs - MEGAHIT
awk '$6~/Viruses/ {sum+=$2} END {print "Total MEGAHIT viral contigs:", sum}' \
    results_short/Taxonomy/contigs/llnl_66ce4dde_MEGAHIT_kraken2_report.txt

# Total viral contigs - SPAdes
awk '$6~/Viruses/ {sum+=$2} END {print "Total SPAdes viral contigs:", sum}' \
    results_short/Taxonomy/contigs/llnl_66ce4dde_SPAdes_kraken2_report.txt

# Total viral contigs (long reads)
awk '$6~/Viruses/ {sum+=$2} END {print "Total long reads viral contigs:", sum}' \
...
1. **nf-core/mag v2.5.4:**
   ...
   - **Only classifies reads, not assembled contigs** (we added custom contig classification)

2. **Long reads direct pipeline:**
   - NanoPlot QC skipped due to NumPy compatibility
   - Direct pipeline (not using nf-core/mag framework)

### Why Two Separate Pipelines?

- nf-core/mag v2.5.4 is stable but doesn't support long-reads-only
- nf-core/mag v3.x supports mixed reads but has strict parameter validation issues
